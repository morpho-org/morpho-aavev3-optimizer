...
import { Types, Events, Errors, MarketLib, MarketBalanceLib, MarketMaskLib, PoolInteractions, InterestRatesModel, WadRayMath, Math, PercentageMath, SafeCast, DataTypes, ReserveConfiguration, UserConfiguration, ThreeHeapOrdering } from "./libraries/Libraries.sol";
import { IPool, IPriceOracleGetter, IVariableDebtToken, IAToken, IPriceOracleSentinel } from "./interfaces/Interfaces.sol";
...
abstract contract MorphoInternal is MorphoStorage {
  ...
  function _decodeId(
    ...
    internal
    pure
    returns (address underlying, Types.PositionType positionType)
  {
  ...
  function _collateralValue(
    ...
    return
      (_marketBalances[poolToken].scaledCollateralBalance(user).rayMul(
        poolSupplyIndex
      ) * underlyingPrice) / tokenUnit; // TODO: Multiply by an index or make collateral balance unscaled
  }
  ...
  function _liquidityData(
    ...
    DataTypes.UserConfigurationMap memory morphoPoolConfig = _pool
    ...
    for (uint256 i; i < poolTokensLength; ++i) {
      ...
      if (
        userMarkets.isSupplyingOrBorrowing(_market[currentMarket].borrowMask)
      ) {
        ...
        Types.AssetLiquidityData
          memory assetLiquidityData = _assetLiquidityData(
            _market[currentMarket].underlying,
            oracle,
            morphoPoolConfig
          );
        ...
        liquidityData.liquidationThresholdValue += liquidityDataSingle
  ...
  function _liquidityDataSingle(
    ...
    if (userMarkets.isSupplying(market.borrowMask)) {
      ...
      liquidityData.maxDebt += assetCollateralValue.percentMul(
        assetLiquidityData.ltv
      );
    }

    // Update debt variable for borrowed token.
    if (amountBorrowed > 0) {
      liquidityData.debt += (amountBorrowed *
        assetLiquidityData.underlyingPrice).divUp(assetLiquidityData.tokenUnit);
    }

    // Update LT variable for withdraw.
    if (assetCollateralValue > 0) {
      liquidityData.liquidationThresholdValue += assetCollateralValue
        .percentMul(assetLiquidityData.liquidationThreshold);
    }

    // Subtract withdrawn amount from liquidation threshold and collateral.
    if (amountWithdrawn > 0) {
      uint256 withdrawn = (amountWithdrawn *
        assetLiquidityData.underlyingPrice) / assetLiquidityData.tokenUnit;
      liquidityData.collateral -= withdrawn;
      liquidityData.liquidationThresholdValue -= withdrawn.percentMul(
        assetLiquidityData.liquidationThreshold
      );
  ...
  function _assetLiquidityData(
    ...
    if (
      !morphoPoolConfig.isUsingAsCollateral(_pool.getReserveData(underlying).id)
    ) {
  ...
  function _setPauseStatus(address poolToken, bool isPaused) internal {
    Types.PauseStatuses storage pauseStatuses = _market[poolToken]
      .pauseStatuses;
  ...
  function _getUserHealthFactor(
    ...
    Types.LiquidityData memory liquidityData = _liquidityData(
      user,
      poolToken,
      withdrawnAmount,
      0
  ...
  function _withdrawAllowed(
    ...
    return
      _getUserHealthFactor(user, poolToken, withdrawnAmount) >=
      HEALTH_FACTOR_LIQUIDATION_THRESHOLD;
  }

  function _liquidationAllowed(
    address user,
    bool isDeprecated
  ) internal view returns (bool liquidationAllowed, uint256 closeFactor) {
    if (isDeprecated) {
      ...
      if (priceOracleSentinel != address(0)) {
        liquidationAllowed = (healthFactor <
