...
contract TestLifecycle is ProductionTest {
...
//     (supply.scaledP2PBalance, supply.scaledPoolBalance) = morpho.supplyBalanceInOf(_market.poolToken, address(user));
...
//     );
//     assertApproxEqAbs(supply.position.total, supply.amount, 1, string.concat(supply.market.symbol, " total supply"));
//     if (supply.p2pDisabled) {
//         assertEq(supply.scaledP2PBalance, 0, string.concat(supply.market.symbol, " borrow delta matched"));
//     } else {
...
//     (supply.position.p2p, supply.position.pool, supply.position.total) =
//         lens.getCurrentSupplyBalanceInOf(supply.market.poolToken, address(user));
// }
...
//     (borrow.scaledP2PBalance, borrow.scaledPoolBalance) = morpho.borrowBalanceInOf(_market.poolToken, address(user));
...
//     );
//     assertApproxEqAbs(borrow.position.total, borrow.amount, 1, string.concat(borrow.market.symbol, " total borrow"));
//     if (borrow.p2pDisabled) {
//         assertEq(borrow.scaledP2PBalance, 0, string.concat(borrow.market.symbol, " supply delta matched"));
//     } else {
...
//     (borrow.position.p2p, borrow.position.pool, borrow.position.total) =
//         lens.getCurrentBorrowBalanceInOf(borrow.market.poolToken, address(user));
// }
...
//     assertEq(borrow.position.p2p, 0, string.concat(borrow.market.symbol, " p2p borrow after repay"));
//     assertEq(borrow.position.pool, 0, string.concat(borrow.market.symbol, " pool borrow after repay"));
//     assertEq(borrow.position.total, 0, string.concat(borrow.market.symbol, " total borrow after repay"));
// }
...
//     assertEq(supply.position.p2p, 0, string.concat(supply.market.symbol, " p2p supply after withdraw"));
//     assertEq(supply.position.pool, 0, string.concat(supply.market.symbol, " pool supply after withdraw"));
//     assertEq(supply.position.total, 0, string.concat(supply.market.symbol, " total supply after withdraw"));
// }
...
//         vm.expectRevert(ErrorsUtils.AmountIsZero.selector);
...
//     for (uint256 marketIndex; marketIndex < activeMarkets.length; ++marketIndex) {
//         vm.expectRevert(ErrorsUtils.AddressIsZero.selector);
...
//         vm.expectRevert(Errors.UserNotMemberOfMarket.selector);
